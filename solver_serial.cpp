...
constexpr int N_SEGS   = 4;  // 0..3: between nodes, 4: last node -> RA
...
bool load_input_file(const std::string& filename,
                     Parameters& par,
                     State& s0,
                     double& t_final,
                     double& dt,
                     ...
                     std::string& inflow_csv)
{
    ...
    while (std::getline(fin, line)) {
        ...
        if (!name.empty() && name[0] == 'C') {
            ...
            if (0 <= idx && idx < N_NODES) par.C[idx] = std::stod(value);
        }
        // ------------------ R_i ------------------
        else if (!name.empty() && name[0] == 'R') {
            // R0, R1, R2, ...
            int idx = std::stoi(name.substr(1));
            if (0 <= idx && idx < N_SEGS) par.R[idx] = std::stod(value);
        }
        // ------------------ L_i ------------------
        else if (!name.empty() && name[0] == 'L') {
            int idx = std::stoi(name.substr(1));
            if (0 <= idx && idx < N_SEGS) par.L[idx] = std::stod(value);
        }
        // ------------------ P0_i ------------------
        else if (name.rfind("P0_", 0) == 0) {
            // P0_i = initial pressure at node i
            int idx = std::stoi(name.substr(3));
            if (0 <= idx && idx < N_NODES) s0.P[idx] = std::stod(value);
        }
        else if (name == "P_RA")      par.P_RA    = std::stod(value);
        else if (name == "T_cycle")   par.T_cycle = std::stod(value);
        else if (name == "t_final")   t_final     = std::stod(value);
        else if (name == "dt")        dt          = std::stod(value);
        else if (name == "use_csv")   use_csv     = (std::stoi(value) != 0);
        else if (name == "inflow_csv") inflow_csv = value;
    }
...
double Q_in_from_csv(double t, const InflowData& inflow)
{
    ...
    // Note: Simple linear search is fine for small data; but this can be improved to binary search.
    std::size_t i = 0;
              ...
              State& ddt)
{
    ...
    // Last node: N_NODES-1, inflow from previous segment, outflow via last segment
...
int main() {
    ...
    if (!load_input_file("./input/input.inp", par, s, t_final, dt, use_csv, inflow_csv_name)) {
        return 1;
    }
    ...
    InflowData inflow;
    if (use_csv) {
        // Build path from $HOME/NERSE570/VascularCircuit/input/<inflow_csv_name> //setme
        const char* home_env = std::getenv("HOME");
        std::string inflow_file;
        if (home_env) {
            std::string home(home_env);
            // Use project-local directory under $HOME
            inflow_file = home + "/NERS570/VascularCircuit0D/input/" + inflow_csv_name; //setme
        } else {
    ...
    // output.csv header
    fout << "t";
    for (int i = 0; i < N_NODES; ++i) fout << ",P_" << i;
    for (int e = 0; e < N_SEGS;  ++e) fout << ",Q_" << e;
    fout << "\n";
    ...
    fout << t;
    for (int i = 0; i < N_NODES; ++i) fout << "," << s.P[i];
    for (int e = 0; e < N_SEGS;  ++e) fout << "," << Qout0[e];
    fout << "\n";
    ...
    for (int n = 0; n < n_steps; ++n) {
        ...
        fout << t;
        for (int i = 0; i < N_NODES; ++i) fout << "," << s.P[i];
        for (int e = 0; e < N_SEGS; ++e)  fout << "," << s.Q[e];
        fout << "\n";
